...
import com.tito.svn.b.po.LogItemInfo;
import com.tito.svn.b.po.SubmitLogInfo;
 
 
public class SvnkitDemo {
	
	private String userName = "huangtt";
	private String password = "huangtt";
	private String urlString = "https://wg772aifvgqhkx6/svn/TestCompManagement/SourceCode/Develop/branch/LogAnalysis";
...
//        System.out.println("startRevision = " + startRevision + ",endRevision = " + endRevision);
     ...
     * @throws Exception
     */
    public int staticticsCodeAdd(File file,SubmitLogInfo submitLogInfo) throws Exception{
...
//					System.out.println("filePath="+changeFile.getFilePath()+"  changeType="+changeFile.getChangeType()+"  addLines="+oneSize);
					...
					if(oneSize > 0)
					{
						logItemInfo = new LogItemInfo();
						logItemInfo.setPath(changeFile.getFilePath());
						logItemInfo.setChangeType(changeFile.getChangeType());
						logItemInfo.setCodeLine(oneSize);
						submitLogInfo.addLogItem(logItemInfo);
						sum += oneSize;
					}
					buffer.setLength(0);
				}
...
//		System.out.println("-----delete file-----"+deleteFile);
	 ...
	 * @throws Exception
	 */
	public List<SubmitLogInfo> staticticsCodeAddByTime(Date startTime, Date endTime) throws Exception{
		List<SubmitLogInfo> logInfs = new ArrayList<SubmitLogInfo>();
		SubmitLogInfo submitLogInfo = null;
		
		int sum = 0;
		SVNLogEntry[] logs = getLogByTime(startTime, endTime);
		if(logs.length > 0){
			...
			for(SVNLogEntry log:logs){
				...
				submitLogInfo = new SubmitLogInfo();
				submitLogInfo.setAuthor(log.getAuthor());
				submitLogInfo.setMessage(log.getMessage());
				submitLogInfo.setRevision(log.getRevision());
				submitLogInfo.setSubmitDate(log.getDate());
				
				
				int addSize = staticticsCodeAdd(logFile, submitLogInfo);
				sum+=addSize;
				lastVersion = log.getRevision();
				
				
				submitLogInfo.setSumCodeLine(addSize);
				logInfs.add(submitLogInfo);
			}
		}
...
//	        @Override
	...
	public static void main(String[] args) {
		...
		Date now = new Date();
		Date twoDayAgo = new Date(now.getTime()-2*24*60*60*1000);//24*60*60*1000
		
		
		try {
//			System.out.println("twoDayAgo="+format.format(twoDayAgo));
//			System.out.println("now="+format.format(now));
			long sum = 0;
			List<SubmitLogInfo>  list = demo.staticticsCodeAddByTime(now, twoDayAgo);
			if(null != list && list.size() > 0 )
			{
				
				for(SubmitLogInfo submitLogInfo : list)
				{
					System.out.println("============================================================================");
					System.out.println("author = " + submitLogInfo.getAuthor());
					System.out.println("Revision = " + submitLogInfo.getRevision());
					System.out.println("SubmitDate = " + format.format(submitLogInfo.getSubmitDate()));
					System.out.println("本次提交有效代码行数 = " + submitLogInfo.getSumCodeLine() + "   ,详情如下：");
					if(null != submitLogInfo.getLogItems() && submitLogInfo.getLogItems().size() > 0 )
					{
						for(LogItemInfo item : submitLogInfo.getLogItems())
						{
							System.out.println(item.getCodeLine() + "<=有效行数   ，  path => " + item.getPath());
						}
					}
					
					sum += submitLogInfo.getSumCodeLine();
				}
				
			}
			System.out.println("本次统计共有效代码行数为： " +  sum);
